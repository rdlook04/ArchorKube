...
- Don't change dependencies: ignore the `npx dt-app update` banner and the `npm install` warnings (skipped install scripts, audit findings). Don't run `dt-app update`, `npm audit fix` or approve install scripts.
...
4. **Configure from what Setup found:**
   ...
   - *Namespaces that aren't teams are excluded*: ask the person which platform namespaces to exclude before trusting ownership coverage. With the namespace provider in the chain, any namespace left in counts as a team, so keep it out of the chain until the exclusions are confirmed. Prefer exact names over substrings.
