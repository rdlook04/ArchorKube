 ...
 */
const clockText = (window: AnalysisWindow, queriedAt: Date): string | undefined => {
  if (window.kind === "range" && window.hours) {
    return formatClockRange(window.hours, queriedAt);
  }
  if (window.kind === "snapshot") return `consultado ${formatClock(queriedAt)}`;
  return undefined;
};
...
export const AnalysisWindowBadge = ({ window, queriedAt }: AnalysisWindowBadgeProps) => {
  ...
  const clock = clockText(window, queriedAt);

  return (
    <Tooltip text={window.detail} placement="bottom">
      <Flex
        alignItems="center"
        gap={8}
        paddingLeft={12}
