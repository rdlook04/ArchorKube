...
const CRITICIDAD_COLORS: Record<string, string> = {
  RIESGO_DISPONIBILIDAD: Colors.Background.Container.Critical.Accent,
  RIESGO_RECURSOS: Colors.Background.Container.Warning.Accent,
  AJUSTE_MENOR: Colors.Background.Container.Neutral.Accent,
  SOLO_SEGURIDAD: Colors.Background.Container.Primary.Accent,
  CUMPLE_TODO: Colors.Background.Container.Success.Accent,
};
...
/**
 * Gráfica categórica apilada: workloads por criticidad de cumplimiento,
 * agrupados por la dimensión elegida (tier/squad/tribu).
 */
export const ComplianceChart = ({ filters }: { filters: TierFilterValue }) => {
  const [dimension, setDimension] = useState<BreakdownDimension>("tier");
  const { data, error, isLoading } = useDql({ query: complianceBreakdown(dimension, filters) });
  ...
  return (
    <Flex flexDirection="column" gap={8}>
      <Flex justifyContent="space-between" alignItems="center" gap={8}>
        <Heading level={4}>Criticidad por {DIMENSION_LABEL[dimension].toLowerCase()}</Heading>
        <Select
